...
def analyse_results(SM_PC, SM_BC, RM_PC, RM_BC):
    ...
    if SM_PC.num_spikes > 0 and SM_BC.num_spikes > 0:  # check if there is any activity
        ...
        plot_raster(spike_times_PC, spiking_neurons_PC, rate_PC, [ISI_hist_PC, bin_edges_PC], True, "blue", multiplier_=1)
...
if __name__ == "__main__":
    ...
    f_in = "wmx_%s_%.1f_linear.pkl"%(STDP_mode, place_cell_ratio)
    ...
    pop, hof, log, hist = opt.run(max_ngen=max_ngen,
                                  cp_filename=os.path.join(base_path, "scripts", "optimization", "checkpoints", "checkpoint_%.1f_%s.pkl"%(place_cell_ratio, STDP_mode)))
    del pool; del opt
